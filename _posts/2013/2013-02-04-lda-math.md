...
layout: post
title: LDA-math-文本建模
date: '2013-02-04T11:13:00.001+08:00'
author: Wenh Q
tags:
modified_time: '2013-11-30T12:25:43.983+08:00'
blogger_id: tag:blogger.com,1999:blog-4961947611491238191.post-1129200970839305385
blogger_orig_url: http://binaryware.blogspot.com/2013/02/lda-math.html
---
...
**4.1 Unigram Model**
假设我们的词典中一共有 $V$ 个词 $v_1, v_2, cdots
v_V$，那么最简单的 Unigram Model
...
上帝的这个唯一的骰子各个面的概率记为 $overrightarrow{p} = (p_1,
p_2, cdots, p_V)$,
所以每次投掷骰子类似于一个抛钢镚时候的贝努利实验， 记为 $wsim
...
对于一篇文档$d=overrightarrow{w}=(w_1, w_2, cdots, w_n)$,
该文档被生成的概率就是

$ p(overrightarrow{w}) = p(w_1, w_2, cdots, w_n) =
p(w_1)p(w_2) cdots p(w_n) $

而文档和文档之间我们认为是独立的，
所以如果语料中有多篇文档 $mathcal{W}=(overrightarrow{w_1},
overrightarrow{w_2},…,overrightarrow{w_m})$,则该语料的概率是
...
$v_i$ 的发生次数 $n_i$，那么 $overrightarrow{n}=(n_1,
n_2,cdots, n_V)$ 正好是一个多项分布
...
p(overrightarrow{p})doverrightarrow{p} $
...
frac{1}{Delta(overrightarrow{alpha})} prod_{k=1}^V
p_k^{alpha_k -1}，
...
int prod_{k=1}^V p_k^{alpha_k -1} doverrightarrow{p} . $
...
prod_{k=1}^V p_k^{n_k + alpha_k -1} doverrightarrow{p}
...
E(overrightarrow{p}) = Bigl(frac{n_1 +
alpha_1}{sum_{i=1}^V(n_i + alpha_i)},
...
hat{p_i} = frac{n_i + alpha_i}{sum_{i=1}^V(n_i +
alpha_i)}

end{equation}

考虑到 $alpha_i$ 在 Dirichlet
...
p(mathcal{W}|overrightarrow{p})
p(overrightarrow{p}|overrightarrow{alpha})doverrightarrow{p}
notag

& = int prod_{k=1}^V p_k^{n_k}
Dir(overrightarrow{p}|overrightarrow{alpha}) doverrightarrow{p}
notag

& = int prod_{k=1}^V p_k^{n_k}
frac{1}{Delta(overrightarrow{alpha})}

prod_{k=1}^V p_k^{alpha_k -1} doverrightarrow{p} notag


& = frac{1}{Delta(overrightarrow{alpha})}

int prod_{k=1}^V p_k^{n_k + alpha_k -1} doverrightarrow{p}
notag
...
**[![topic-examples](http://www.52nlp.cn/wp-content/uploads/2013/02/topic-examples.jpg)](http://www.52nlp.cn/lda-math-%e6%96%87%e6%9c%ac%e5%bb%ba%e6%a8%a1/topic-examples)Topic
就是Vocab 上的概率分布**
...
overrightarrow{varphi}_K$, 对于包含$M$篇文档的语料 $C=(d_1,
d_2, cdots, d_M)$
中的每篇文档$d_m$，都会有一个特定的doc-topic骰子$overrightarrow{theta}_m$，所有对应的骰子记为
$overrightarrow{theta}_1, cdots,
overrightarrow{theta}_M$。为了方便，我们假设每个词$w$
都是一个编号，对应到topic-word 骰子的面。于是在 PLSA
这个模型中，第$m$篇文档 $d_m$ 中的每个词的生成概率为

$ p(w|d_m) = sum_{z=1}^K p(w|z)p(z|d_m) = sum_{z=1}^K
varphi_{zw} theta_{mz}$

所以整篇文档的生成概率为

$ p(overrightarrow{w}|d_m) = prod_{i=1}^n sum_{z=1}^K
p(w_i|z)p(z|d_m) =

prod_{i=1}^n sum_{z=1}^K varphi_{zw_i} theta_{dz} $
...
![](http://feeds.feedburner.com/~r/52nlp/~4/q2JrE6Dni_0)

...
default character set utf8 collate utf8\_general\_ci;\
\#删除数据库drop database examples;\
\#创建表create table test(\
    id int(10) unsigned zerofill not null auto\_increment,\
...
describe \
\#删除表drop table test；\
\#重命名表alter table test\_old rename test\_new;\
...
时分秒\
time\_to\_sec(now()),sec\_to\_time(3600\*8);\#转换时间为秒和还原\
...
\$r = mysql\_query("SELECT count(\*) FROM user");\
 ...
                    \$r = mysql\_query("SELECT username FROM user LIMIT
\$rand, 1");7.避免使用select

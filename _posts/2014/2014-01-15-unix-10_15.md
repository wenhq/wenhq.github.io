...
3 中的示例显示可以如何将此操作符与 `!$` 运算符组合使用。在第一个命令中，将一个文件重新命名为更有意义的名称，但为了保持原始文件名可用，创建了一个符号链接。重新命名文件 *kxp12.c* 以提高可读性，然后使用 `link` 命令来创建到原始文件名的符号链接，以防在其他位置使用该文件名。`!$` 操作符返回 *file_system_access.c* 文件名，而 `!:1` 操作符返回 *kxp12.c* 文件名，该文件名是上个命令的第一个文件名。

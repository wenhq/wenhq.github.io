...
author: Wenh Q
tags: NLP - tech.nlp
modified\_time: '2013-11-30T12:25:43.983+08:00' 
blogger\_id: tag:blogger.com,1999:blog-4961947611491238191.post-1129200970839305385
...
overrightarrow{p} = (p\_1,
p\_2, 

cdots, p\_V)
$,
所以每次投掷骰子类似于一个抛钢镚时候的贝努利实验， 记为 
$w
...
height="132"}](http://www.52nlp.cn/lda-math-%e6%96%87%e6%9c%ac%e5%bb%ba%e6%a8%a1/unigram-model)上帝投掷
$V
$
个面的骰子**
...
overrightarrow{p}
$，也就是问上帝拥有的这个骰子的各个面的概率是多大，按照统计学家中频率派的观点，使用最大似然估计最大化
$P(
...
overrightarrow{p}
$不是唯一固定的，它也是一个随机变量。所以按照贝叶斯学派的观点，上帝是按照以下的过程在玩游戏的
...
mathcal{W}
$产生的概率如何计算呢？由于我们并不知道上帝到底用了哪个骰子
$

overrightarrow{p}
$,所以每个骰子都是可能被使用的，只是使用的概率由先验分布
$p(
...
overrightarrow{p}
$如何估计呢？由于我们已经有了参数的后验分布，所以合理的方式是使用后验分布的极大值点，或者是参数在后验分布下的平均值。在该文档中，我们取平均值作为参数的估计值。使用上个小节中的结论，由于
...
bag-of-words 模型。游戏中的
$K
$ 个topic-word 骰子，我们可以记为
